...
import { TranscodeEncoding } from "node:buffer";
...
export async function sandboxController(room: Room, ...args: any) {
	...
	let starInvokeHandler: string|null = null;
	const aliases: Map<string, [string, string]> = new Map();
	function registerAlias(alias: string, module: string, functionName: string){
	...
	// add events
	eventEmitter.on("setPublic", (value: boolean) => {
		room.setPublicType(value ? publicType : null);
	});
	eventEmitter.on("send", (target: string|string[], ...message: unknown[]) => {
