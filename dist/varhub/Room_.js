...
export class Room_ extends EventEmitter {
    ...
    async _init({ modules: moduleDescriptors, config, integrityRequired, integrity }) {
        ...
        if (integrity != null && integrity !== this.#integrity)
        ...
        const hooks = {
            ...
            setPublic: (flag) => this.#setPublic(Boolean(flag)),
            setPublicMessage: (message) => this.#setPublicMessage(JSON.parse(JSON.stringify(message))),
        };
